...
namespace BasicOperation.Dialog.BarcodeOption.Zebra
{
    public partial class OptionCode93Dialog : Form
    {
            ...
            : base()
        {
            ...
            this.mReader = reader;
            this.mSymbol = symbol;
            if (symbol == ParamName.D2of5)
                this.Text = "Discrete 2 of 5 Configuration";
            else
        ...
        private void BeginLoadParam(Object obj)
        {
            ...
            if(mSymbol == ParamName.Code93)
                values = mReader.getBarcode().getBarcodeParam(new ParamName[] { ParamName.Code93Length1, ParamName.Code93Length2 });
            else if(mSymbol == ParamName.D2of5)
                values = mReader.getBarcode().getBarcodeParam(new ParamName[] { ParamName.D2of5Length1, ParamName.D2of5Length2 });
        ...
        private void EndLoadParam(Object obj)
        {
            ...
            {
                if(mSymbol == ParamName.Code93)
                    SetLength(values.GetValue(ParamName.Code93Length1), values.GetValue(ParamName.Code93Length2));
                else if(mSymbol == ParamName.D2of5)
                    SetLength(values.GetValue(ParamName.D2of5Length1), values.GetValue(ParamName.D2of5Length2));
            }
            catch (Exception e)
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (mSymbol == ParamName.Code93)
            {
                values.Add(ParamName.Code93Length1, (int)numMin.Value);
                values.Add(ParamName.Code93Length2, (int)numMax.Value);
            }
            else if (mSymbol == ParamName.D2of5)
            {
                values.Add(ParamName.D2of5Length1, (int)numMin.Value);
                values.Add(ParamName.D2of5Length2, (int)numMax.Value);
            }
            else
            {
